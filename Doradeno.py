...
for fold, (train_index, test_index) in enumerate(skf.split(sequences, labels)):
    ...
    val_sequences, temp_sequences, val_labels, temp_labels = train_test_split(
        train_sequences, train_labels, test_size=0.125, random_state=42, stratify=train_labels)
    
    batch_size = 256

    train_sequences, val_sequences = train_sequences, val_sequences
    ...
    model_file_path = f'Models/model_file_fold_{fold}.bin'
    ...
    if not os.path.exists(model_file_path):
        ...
        plt.legend()
        plt.show()
        plt.savefig(f'Training_plot_fold_{fold}.png')
...
std_roc_auc_score = np.std(roc_auc_scores)

# Print results
print("Cross-validation results:")
print(f"Mean F1 Score: {mean_f1_score}, Std F1 Score: {std_f1_score}")
print(f"Mean ROC AUC Score: {mean_roc_auc_score}, Std ROC AUC Score: {std_roc_auc_score}")
